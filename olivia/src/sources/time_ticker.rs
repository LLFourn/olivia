...
use olivia_core::{Event, EventKind, Outcome, StampedOutcome};
use tokio::time;

pub struct TimeEventStream {
    pub db: Arc<dyn DbReadEvent>,
...
impl TimeOutcomeStream {
    pub fn start(self) -> impl stream::Stream<Item = Update<StampedOutcome>> {
        ...
        tokio::spawn(async move {
            loop {
                let event = db .earliest_unattested_child_event("/time", EventKind::SingleOccurrence) .await;
