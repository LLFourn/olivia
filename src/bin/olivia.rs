...
use olivia::{
    ...
    db::{diesel::postgres::PgBackend, Db},
    event::{Event, Outcome},
    oracle::Oracle,
    sources::Update,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db: Arc<dyn Db> = match config.database {
        DbConfig::InMemory => Arc::new(olivia::db::in_memory::InMemory::default()),
    ...
    if let Some(rest_config) = config.rest_api {
        let rest_api_server = warp::serve(olivia::rest_api::routes(db.clone()))
